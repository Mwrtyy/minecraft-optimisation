...
### Build executable (.exe / native binary)
```bash
npm install
npm run build:exe
```

This generates binaries in `dist-exe/` using `esbuild` + `pkg`:
- `contested-lane-server.exe` (Windows target)
- `contested-lane-server` (Linux target)

Keep `config/game-config.json` next to the executable (or set `GAME_CONFIG_PATH`).
